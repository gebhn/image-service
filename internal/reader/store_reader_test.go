...
func (s *storeMock) List(ctx context.Context, prefix string) ([]string, error) {
	return []string{"image_1", "image_2"}, nil
}
...
func TestStoreReader(t *testing.T) {
	...
	t.Run("should read many", func(t *testing.T) {
		...
		assert.Equal(t, us[0], appendDelimiter(config.GetCloudFrontURL())+"image_1")
		assert.Equal(t, us[1], appendDelimiter(config.GetCloudFrontURL())+"image_2")
	})
}
